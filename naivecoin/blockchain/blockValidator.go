...
//IsValidNewBlock : Checks if new block is valid regarding the latest block. Does NOT validate transactions.
func IsValidNewBlock(newBlock *Block, latestBlock *Block, currentTimestamp time.Time) (bool, error) {
	...
	if expectedHash, err := newBlock.CalculateHash(); newBlock.Hash != expectedHash {
		return false, err
	}
